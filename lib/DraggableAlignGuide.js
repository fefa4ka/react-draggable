...
export default class DraggableAlignGuide extends React.Component {
  ...
  static propTypes = {
    ...
    snapTreshhold: PropTypes.number,
  ...
  renderGuide = (item = {}, index) => {
    const { axis, position, additionalClass } = item;
    ...
    return (<div key={index} className={className} style={_styles} />)
  }
  renderAxis = () => {
    ...
    if (axis && axis.length) {
      return axis.map((item, index) => {
        if (item) {
          return this.renderGuide(item, index)
        }
      })
    }
